...
import "./Settings.sol";
import "./ResumeTokenVault.sol";

contract ERC721VaultFactory is Ownable, Pausable ,IERC721Receiver{
  ...
  /// @notice the ResumeTokenVault resumevault contract
  address public immutable resumevault;

  event Mint(address indexed token, uint256 id, uint256 price, address vault, uint256 vaultId);

  constructor(address _settings) {
    settings = _settings;
    resumevault = address(new ResumeTokenVault(_settings));
  }
