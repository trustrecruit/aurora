...
contract ResumeTokenVault is ERC20Upgradeable, ERC721HolderUpgradeable {
    ...
    //    // require(msg.sender == Ownable(settings).owner(), "remove:not gov");
    //     require(auctionState == State.live, "update:token is not present!");
    //     uint256 curator_price = userPrices[curator];
    //     uint256 netprice = curator_price*(amount)/(votingTokens);
    //     uint256 finalprice = curator_price-netprice;
    //     require(votingTokens >= amount, "buy:enter low quantity of token");
    //     require(msg.value >= netprice, "buy:too low bid");
    //     require(0 != curator_price, "buy:no price specified");
    //     // uint256 weight = balanceOf(curator);

    //      votingTokens -= amount;
    //      reserveTotal -=  netprice;
    //      userPrices[curator] = finalprice;
    //      _sendETHOrWETH(payable(curator), amount);
    //      transfer(msg.sender,amount);
    //     emit PriceUpdate(curator, finalprice);
    // }
...
// function transferFrom(address from, address to, uint256 amount) override virtual public returns (bool) {
        // if(_transactionMaxValue > 0){
        //  require(_transactionMaxValue >= amount, "You can not transfer more than 1000000 tokens at once!");
        // }
        // added `return super.`
 ...
 //     require(balanceOf(address(this))>=_amount,"update:balance is not ");
 //     transfer(msg.sender, _amount);

 // }
// function transferMytokenSuper() external{
//      super.transferFrom(curator, msg.sender, balanceOf(curator));
//  }

//  function transferMytokenThis() external{
//      this.transferFrom(curator, msg.sender, balanceOf(curator));
//  }

//  function transferToken() external{
//      transfer(address(this), balanceOf(address(this)));
//  }
//  function swap(IERC20 token1,address recipient,uint256 amount1) public {
       
//         require(
//             token1.allowance(msg.sender, address(this)) >= amount1,
//             "Token1 allowance too low"
//         );
       

//         _safeTransferFrom(token1, msg.sender, recipient, amount1);
...
//         uint amount
//     ) private {
//         bool sent = token1.transferFrom(sender, recipient, amount);
